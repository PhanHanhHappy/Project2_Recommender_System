...
def recommend_products_for_new_user_with_svd(user_id, model_svd, df_products, top_n=3):
    """
...
def simple_hybrid_recommendation(user_id, danh_gia, df_products, cosine_sim_new, model_svd, top_n=3, weight_content=0.5):
    """
   ...
   # Chia phần 1 thành 5 mục nhỏ
    ...
    if user_id:
        if is_new_user(user_id):
            st.write("Người dùng mới, gợi ý sản phẩm cho người dùng có user_id:", user_id)
            recommendations = recommend_products_for_new_user_with_svd(user_id, model_svd, df_products, top_n=3)
            st.dataframe(recommendations)
        else:
            st.write("Người dùng cũ, gợi ý sản phẩm cho người dùng có user_id:", user_id)
            recommendations = recommend_products_for_old_user(user_id, model_svd, st.session_state.danh_gia2, df_products, top_n=3)
            st.dataframe(recommendations)
    ...
    if user_id:
        try:
            ...
            if not recommendations_df.empty and not additional_info_df.empty:
                st.write(f"Các sản phẩm gợi ý cho người dùng có user_id: {user_id}")  # Hiển thị user_id
                # Hiển thị từng sản phẩm trong một cột
                additional_info_df=additional_info_df.head(3)
                num_products = len(additional_info_df)  # Lấy số lượng sản phẩm
                cols = st.columns(num_products)  # Tạo số cột tương ứng
                for i, product in additional_info_df.iterrows():
                    with cols[i % 3]:
                        st.write(product['ten_san_pham'])
                        st.write(f"Giá bán: {product['gia_ban']}")
